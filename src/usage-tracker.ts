...
export class UsageTracker {
	...
	/**
	 * Process a raw JSONL line directly (called from JsonlParser extensions).
	 * Returns an event type + data if a usage event should be emitted, null otherwise.
	 */
	processRawLine(
		_sessionId: string,
		...
		// Rate limit events
		if (subtype === "rate_limit" || this.isRateLimitLine(line)) {
			return this.buildRateLimitEvent(line);
		}

		// Context window events
		if (subtype === "context_window" || this.isContextWindowLine(line)) {
			return this.buildContextWindowEvent(line);
		}
	...
	private isContextWindowLine(line: Record<string, unknown>): boolean {
		return (
			"context_window" in line || "tokens_used" in line || "tokens_max" in line
		);
	}

	private buildRateLimitEvent(line: Record<string, unknown>): {
		type: string;
		data: Record<string, unknown>;
	} {
		const allowed =
	...
	private buildContextWindowEvent(line: Record<string, unknown>): {
		type: string;
		data: Record<string, unknown>;
	} {
		...
		if (typeof contextWindow === "object" && contextWindow !== null) {
			...
			// Flat format
			tokensUsed = extractNumber(line, "tokens_used") ?? 0;
			tokensMax = extractNumber(line, "tokens_max") ?? 0;
			if (tokensMax > 0) percentUsed = (tokensUsed / tokensMax) * 100;
			const percentRaw = extractNumber(line, "percent_used");
