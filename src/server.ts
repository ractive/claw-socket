...
import { ClientMessageSchema, type Snapshot, type EventEnvelope } from "./schemas/index.ts";
  ...
  const server = Bun.serve<ClientData>({
    ...
    fetch(req, server) {
      ...
      if (url.pathname === "/health") {
        return new Response(JSON.stringify({ status: "ok", sessions: discovery.getSessions().length }), {
    ...
    websocket: {
      ...
      message(ws, message) {
        ...
        if (!result.success) {
          ws.send(JSON.stringify({ error: "invalid message", details: result.error.issues }));
          return;
        }
        ...
        switch (msg.type) {
          case "subscribe":
            ...
            ws.send(JSON.stringify({ type: "subscribed", topics: Array.from(ws.data.subscriptions) }));
            break;
          ...
          case "unsubscribe":
            ...
            ws.send(JSON.stringify({ type: "unsubscribed", topics: Array.from(ws.data.subscriptions) }));
            break;
