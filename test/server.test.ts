...
describe("WebSocket server", () => {
  test("sends snapshot on connect", async () => {
    ...
    expect(msg).toMatchObject({ type: "snapshot", sessions: expect.any(Array) });
    ws.close();
  });
  ...
  test("unsubscribe returns confirmation", async () => {
    ...
    ws.send(JSON.stringify({ type: "subscribe", topics: ["session.*", "tool.*"] }));
    await waitForMessage(ws);
  ...
  test("broadcast only reaches subscribed clients", async () => {
    ...
    app.broadcast(envelope("session.discovered", "test-session", { pid: 9999 }));
